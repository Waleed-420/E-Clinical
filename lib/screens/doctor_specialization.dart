...
Future<bool> updateSpecializationOnServer() async {
  ...
  final url = Uri.parse('http://192.168.1.4:5000/api/doctor/$doctorId/specialization');
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text('Your Specialization', style: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
              const SizedBox(height: 16),
              Text('Please select your medical specialization from the list below.', style: theme.textTheme.bodyMedium),
    ...
    : () async {
        ...
        if (updated) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Failed to update specialization')),
          );
        }
      },

                  style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16)),
child: Text(_selectedSpecialization == null ? 'Continue' : 'Change'),
                ),
              ),
            ],
          ),
