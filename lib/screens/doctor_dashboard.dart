...
class _DoctorDashboardState extends State<DoctorDashboard> {
  ...
  Future<void> _fetchUpcomingAppointments() async {
    try {
      final response = await http.get(
        Uri.parse(
          'http://192.168.1.6:5000/api/appointments?doctorId=${widget.user['_id']}&status=booked',
        ),
      );
  ...
  Future<void> _fetchDoctorData() async {
    try {
      final response = await http.get(
        Uri.parse(
          'http://192.168.1.6:5000/api/doctors?doctorId=${widget.user['_id']}',
        ),
      );
                      ...
                      onPressed: () async {
                        if (_newFee >= 500 && _newFee <= 2500) {
                          final response = await http.post(
                            Uri.parse(
                              'http://192.168.1.6:5000/api/doctor/${widget.user['_id']}/fee',
                            ),
