...
class _UserAppointmentsState extends State<UserAppointments> {
  ...
  Future<void> fetchAppointments() async {
    try {
      final response = await http.get(
        Uri.parse('http://192.168.1.4:5000/api/user/${widget.user['_id']}/appointments'),
      );
  ...
  Widget _buildUserFeatures(BuildContext context, Map<String, dynamic> appointment) {
    return Column(
        ...
        Wrap(
          ...
          children: [
            _buildFeatureButton(
              ...
              'Chat',
              Colors.blue,
              () => _showFeatureDialog(context, 'Chat with ${appointment['otherName']}'),
            ),
            _buildFeatureButton(
              context,
              ...
              'Prescription',
              Colors.green,
              () => _showFeatureDialog(context, 'View prescription from ${appointment['otherName']}'),
            ),
            _buildFeatureButton(
              context,
              Icons.upload_file,
              'Send Reports',
              Colors.orange,
              () => _showFeatureDialog(context, 'Send medical reports to ${appointment['otherName']}'),
            ),
            _buildFeatureButton(
              context,
              Icons.star,
  ...
  // Feature for Doctors
  Widget _buildDoctorFeatures(BuildContext context, Map<String, dynamic> appointment) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const SizedBox(height: 12),
        ...
        Wrap(
          ...
          children: [
            ...
            _buildFeatureButton(
              ...
              'Video Call',
              Colors.purple,
              () => _showFeatureDialog(context, 'Start video consultation with ${appointment['otherName']}'),
            ),
            _buildFeatureButton(
              context,
              Icons.download,
              'Request Reports',
              Colors.teal,
              () => _showFeatureDialog(context, 'Request medical reports from ${appointment['otherName']}'),
            ),
            _buildFeatureButton(
              context,
              Icons.medical_services,
              'Prescribe',
              Colors.green,
              () => _showFeatureDialog(context, 'Write prescription for ${appointment['otherName']}'),
            ),
          ],
        ),
      ],
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Thanks for rating ${appointment['otherName']} with $rating stars!'),
                      ),
                    );
